...
// Uses Resend (resend.com) instead of raw SMTP — works on Render without port issues.
// npm install resend
...
// After verifying schoolgatess.com in Resend DNS settings, change to:
//   supports@schoolgatess.com
const FROM = process.env.FROM_EMAIL
  ? `${process.env.FROM_NAME || 'ScholarsGate'} <${process.env.FROM_EMAIL}>`
  : 'ScholarsGate <onboarding@resend.dev>';
...
const templates = {
    ...
    html: `
      <div style="font-family:Georgia,serif;max-width:600px;margin:0 auto;background:#0a0e1a;color:#e8e4d9;padding:40px;border-radius:12px;">
        ...
        <p style="color:#aaa;line-height:1.8;margin-bottom:24px;">
          Your ScholarsGate account has been created. You're one step closer to accessing elite USA high school
          scholarship opportunities for your child.
        </p>
        <a href="${data.verifyUrl}"
          style="display:inline-block;background:linear-gradient(135deg,#c9a84c,#e8c96a);color:#0a0e1a;
          padding:14px 32px;border-radius:6px;text-decoration:none;font-weight:bold;letter-spacing:1px;">
          VERIFY EMAIL
        </a>
  ...
  applicationStatusUpdate: (data) => {
    ...
    const contextMessages = {
      ...
      shortlisted:         'Congratulations! Your child has been shortlisted for this scholarship. We will reach out with next steps shortly.',
      interview_scheduled: 'An interview has been scheduled. Please check your dashboard for details and prepare accordingly.',
      interview_done:      'The interview stage has been completed. Our team is deliberating and will notify you of the outcome soon.',
      accepted:            '🎊 Wonderful news! Your child\'s application has been accepted. Please log in to review and accept the offer letter.',
      rejected:            'After careful consideration, your child\'s application was not selected at this time. We encourage you to explore other scholarship opportunities.',
      waitlisted:          'Your child has been placed on the waitlist. We will notify you immediately if a place becomes available.',
      withdrawn:           'Your application has been marked as withdrawn. If this was in error, please contact our support team.',
    };

    const contextMessage = contextMessages[data.status]
      || 'Your application status has been updated. Please log in for full details.';
    ...
    return {
      ...
      html: `
        ...
        <body style="margin:0;padding:0;background:#f5f4f0;font-family:Georgia,serif;">
          <div style="max-width:600px;margin:40px auto;background:#fff;border-radius:16px;overflow:hidden;box-shadow:0 8px 32px rgba(0,0,0,0.12);">

            <div style="background:linear-gradient(135deg,#0a0e1a,#141c35,#1a2448);padding:40px 40px 32px;text-align:center;">
              <div style="font-size:28px;font-weight:700;color:#c9a84c;letter-spacing:2px;">ScholarsGate</div>
              <div style="font-size:11px;letter-spacing:4px;text-transform:uppercase;color:#888;margin-top:4px;">Elite USA Admissions</div>
            </div>
            ...
            <div style="background:${statusColor};padding:20px 40px;text-align:center;">
              ...
              <div style="font-size:20px;font-weight:700;color:#fff;letter-spacing:1px;text-transform:uppercase;">${statusLabel}</div>
            </div>
            ...
            <div style="padding:40px;">
              ...
              <div style="text-align:center;margin-bottom:28px;">
                <a href="${data.dashboardUrl}"
                  style="display:inline-block;background:linear-gradient(135deg,#c9a84c,#e8c96a);color:#0a0e1a;
                  padding:16px 40px;border-radius:8px;text-decoration:none;font-weight:700;
