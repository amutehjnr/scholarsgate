...
const Student     = require('../models/Student');
const Application = require('../models/Application');
const { Offer, Payment, AuditLog } = require('../models/index');
const { sendEmail }                 = require('../utils/email');
const AppError                      = require('../utils/AppError');
const logger                        = require('../utils/logger');
...
const resolveSegment = async (segment) => {
  switch (segment) {
    ...
    case 'offer_expired': {
      const expiredIds = await Offer.distinct('guardian', {
        status:     'issued',
        expiryDate: { $lt: new Date() },
      });
      return Guardian.find({ isActive: true, _id: { $in: expiredIds } })
        .select('firstName lastName email').lean();
    }
    ...
    case 'accepted': {
      // Applications with status "accepted" — parent needs to know
      const acceptedGuardianIds = await Application.distinct('guardian', {
        status: 'accepted',
      });
      return Guardian.find({ isActive: true, _id: { $in: acceptedGuardianIds } })
        .select('firstName lastName email').lean();
    }

    case 'offer_issued': {
      // Guardians who have an offer letter issued but haven't accepted yet
      // and offer hasn't expired yet
      const offerIssuedIds = await Offer.distinct('guardian', {
        status:     'issued',
        expiryDate: { $gt: new Date() },
      });
      return Guardian.find({ isActive: true, _id: { $in: offerIssuedIds } })
        .select('firstName lastName email').lean();
    }

    case 'deposit_pending': {
      // Guardians who accepted their offer but have NOT submitted
      // an enrollment deposit payment yet (or payment is pending/rejected)
      const acceptedOfferIds = await Offer.distinct('_id', {
        status: { $in: ['accepted', 'confirmed'] },
      });
      // Get guardian IDs that already have a verified deposit
      const verifiedGuardianIds = await Payment.distinct('guardian', {
        offer:       { $in: acceptedOfferIds },
        paymentType: 'enrollment_deposit',
        status:      'verified',
      });
      // Get all guardians with accepted offers
      const acceptedGuardianIds = await Offer.distinct('guardian', {
        status: { $in: ['accepted', 'confirmed'] },
      });
      // Subtract those who already paid
      return Guardian.find({
        isActive: true,
        _id: { $in: acceptedGuardianIds, $nin: verifiedGuardianIds },
      }).select('firstName lastName email').lean();
    }
...
// ─────────────────────────────────────────────────────────────────────────────
// SANITIZE BODY
// Decodes HTML entities and fixes markdown artifacts before sending
// ─────────────────────────────────────────────────────────────────────────────
const sanitizeBody = (html) => {
  let out = html
    .replace(/&lt;/g,   '<')
  ...
  out = out.replace(/\[([^\]]+)\]\(([^)]+)\)/g, '<a href="$2">$1</a>');
  // Fix corrupted href: href="[text](url)" → href="url"
  out = out.replace(/href="[^"]*\[([^\]]+)\]\(([^)]+)\)[^"]*"/g, 'href="$2"');
...
// BRANDED EMAIL WRAPPER
// ─────────────────────────────────────────────────────────────────────────────
const wrapInTemplate = ({ firstName, subject, previewText, bodyHtml, accentColor }) => {
...
<head>
  ...
  <meta name="viewport" content="width=device-width,initial-scale=1.0"/>
  <title>${subject}</title>
  <style>body{margin:0;padding:0;background:#f5f4f0;font-family:Georgia,serif;}a{color:${accent};}</style>
</head>
<body style="margin:0;padding:0;background:#f5f4f0;">
  ...
  <table width="100%" cellpadding="0" cellspacing="0" border="0" style="background:#f5f4f0;padding:32px 16px;">
    <tr><td align="center">
      <table width="600" cellpadding="0" cellspacing="0" border="0"
        ...
        <!-- Accent rule -->
        <tr><td style="height:3px;background:linear-gradient(90deg,${accent},#e8c96a,${accent});"></td></tr>
        ...
        <tr>
          <td align="center" style="padding:0 40px 32px;">
            <a href="${process.env.APP_URL}/parent/dashboard"
              style="display:inline-block;background:linear-gradient(135deg,${accent},#e8c96a);
              color:#0a0e1a;padding:14px 36px;border-radius:8px;text-decoration:none;
              font-weight:700;font-size:13px;letter-spacing:2px;text-transform:uppercase;
              font-family:'Jost',sans-serif;">
              Go to My Dashboard
        ...
        <tr>
          <td style="padding:0 40px 28px;">
            <p style="font-size:14px;color:#4a4a6a;margin:0 0 4px;">Warm regards,</p>
            <p style="font-size:14px;font-weight:700;color:#1a1a2e;margin:0;">The ScholarsGate Admissions Team</p>
            <p style="font-size:12px;color:${accent};margin:4px 0 0;">supports@schoolgatess.com</p>
          </td>
        </tr>
        ...
        <tr>
          <td style="background:#0a0e1a;padding:24px 40px;border-radius:0 0 16px 16px;">
            ...
            <p style="font-size:11px;color:#333;margin:6px 0 0;text-align:center;">
              Questions? <a href="mailto:supports@schoolgatess.com" style="color:#c9a84c;text-decoration:none;">supports@schoolgatess.com</a>
            </p>
          </td>
        </tr>
...
exports.getCampaignPage = async (req, res) => {
  ...
  // Counts for new segments
  const acceptedIds = await Application.distinct('guardian', { status: 'accepted' });

  const activeOfferIds   = await Offer.distinct('guardian', { status: 'issued', expiryDate: { $gt: new Date() } });
  const acceptedOfferIds = await Offer.distinct('_id',      { status: { $in: ['accepted', 'confirmed'] } });
  const verifiedPayIds   = await Payment.distinct('guardian', {
    offer: { $in: acceptedOfferIds }, paymentType: 'enrollment_deposit', status: 'verified',
  });
  const acceptedOfferGuardianIds = await Offer.distinct('guardian', { status: { $in: ['accepted', 'confirmed'] } });

  const [acceptedCount, offerIssuedCount, depositPendingCount] = await Promise.all([
    Guardian.countDocuments({ isActive: true, _id: { $in: acceptedIds } }),
    Guardian.countDocuments({ isActive: true, _id: { $in: activeOfferIds } }),
    Guardian.countDocuments({ isActive: true, _id: { $in: acceptedOfferGuardianIds, $nin: verifiedPayIds } }),
  ]);
  ...
  const segments = [
    ...
    // ── Application & offer segments ─────────────────────────────────────────
    {
      id:          'accepted',
      label:       'Application Accepted',
      description: 'Parents whose child\'s application has been accepted. Congratulate and guide next steps.',
    ...
    {
      id:          'offer_issued',
      label:       'Offer Letter Issued — Not Yet Accepted',
      description: 'Parents who have received an offer letter but have not accepted it yet.',
      icon:        'fa-envelope-open-text',
      color:       'gold',
      count:       offerIssuedCount,
      group:       'Applications',
    },
    {
      id:          'offer_expired',
    ...
    {
      id:          'deposit_pending',
      label:       'Enrollment Deposit Not Paid',
      description: 'Parents who accepted their offer but have not yet submitted the enrollment deposit.',
      icon:        'fa-coins',
      color:       'warning',
...
exports.sendCampaign = async (req, res, next) => {
  ...
  if (type === 'individual') {
    ...
    try {
      ...
      logger.error(`Individual email failed for ${guardian.email}: ${err.message}`);
      return res.json({ success: false, message: 'Email failed to send. Check SMTP settings.' });
    }
  }
